...
remaining $A$ after $t$ generation, and it is: \$ P_t (A ) = 1/4
+3/4(1-4/3\alpha)\^t \$. For $t=0$, the probability is 1, as it should
be, and as $t \rightarrow \infty$, $P_t (A) \rightarrow 1/4$, since this
is the equilibrium probability distribution. Note that the expression is
the same for all the other letters, so we have found the expression for
any nucleotide remaining the same after $t$ generations.
...
$\log (1 -4/3 \alpha) \approx - 4/3\alpha$. Using the formula for $t$
from above with this approximation, we find the Jukes-Cantor
...
$m = 0$, $d_{JC} = 0$ (identical sequences have zero distance), and when
